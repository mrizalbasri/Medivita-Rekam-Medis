...
type VisitType = "Konsultasi" | "Pemeriksaan Lab" | "Radiologi" | "Rawat Inap" | "Farmasi";
...
function getTypeConfig(type: VisitType) {
  switch (type) {
    case "Konsultasi":
      return { icon: <StethoscopeIcon />, bg: "bg-primary-soft", text: "text-primary", dot: "bg-primary" };
    case "Pemeriksaan Lab":
      return { icon: <FlaskIcon />, bg: "bg-accent-soft", text: "text-accent", dot: "bg-accent" };
    case "Radiologi":
      return { icon: <XRayIcon />, bg: "bg-[#f0e6ff]", text: "text-[#7c3aed]", dot: "bg-[#7c3aed]" };
    case "Rawat Inap":
      return { icon: <HospitalIcon />, bg: "bg-[#fff7ed]", text: "text-[#ea580c]", dot: "bg-[#ea580c]" };
    case "Farmasi":
      return { icon: <PillIcon />, bg: "bg-[#f0fdf4]", text: "text-[#16a34a]", dot: "bg-[#16a34a]" };
  }
}
...
function getStatusConfig(status: VisitStatus) {
  switch (status) {
    case "Selesai":
      return { cls: "bg-accent-soft text-accent border-accent/20", label: "Selesai" };
    case "Dalam Proses":
      return { cls: "bg-[#fff7ed] text-[#ea580c] border-[#ea580c]/20", label: "Dalam Proses" };
    case "Dibatalkan":
      return { cls: "bg-alert-soft text-alert border-alert/20", label: "Dibatalkan" };
  }
}
...
function VisitCard({ visit, index }: { visit: VisitRecord; index: number }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Type Icon */}
        <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${typeConf.bg} ${typeConf.text}`}>
          {typeConf.icon}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-0.5">
            <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-bold ${typeConf.bg} ${typeConf.text} border-transparent`}>
              {visit.tipeKunjungan}
            </span>
            <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-bold ${statusConf.cls}`}>
              {statusConf.label}
            </span>
          </div>
          <h3 className="font-display text-sm font-bold text-primary-dark truncate">{visit.faskes}</h3>
          <p className="text-xs text-ink-soft mt-0.5 truncate">{visit.dokter} · {visit.spesialisasi}</p>
        </div>
        ...
        <div className="flex items-center gap-4 shrink-0">
          ...
          <div className="hidden md:block text-right">
            ...
            <p className="text-sm font-bold text-primary-dark">{formatCurrency(visit.biaya)}</p>
          </div>
          <ChevronDownIcon
      ...
      <div className="sm:hidden px-5 pb-3 -mt-2 flex items-center gap-3 text-xs text-ink-soft">
        <span className="flex items-center gap-1"><CalendarIcon className="h-3.5 w-3.5" />{formatDate(visit.tanggal)}</span>
        <span className="flex items-center gap-1"><ClockIcon className="h-3.5 w-3.5" />{visit.waktu} WIB</span>
      </div>
      ...
      {expanded && (
        <div className="border-t border-line px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-5 animate-[fade-in-up_0.2s_ease-out_both]">
          ...
          <div className="space-y-4">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-ink-soft mb-1.5">Keluhan Utama</p>
              <p className="text-sm text-ink leading-relaxed">{visit.keluhan}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-ink-soft mb-1.5">Diagnosis</p>
              <p className="text-sm font-semibold text-primary-dark leading-relaxed">{visit.diagnosis}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-ink-soft mb-1.5">Tindakan / Pemeriksaan</p>
              <p className="text-sm text-ink leading-relaxed">{visit.tindakan}</p>
            </div>
            <div className="flex items-center gap-1.5 text-xs text-ink-soft">
              <MapPinIcon className="h-3.5 w-3.5 shrink-0" />
              <span>{visit.faskes} · {visit.lokasiKota}</span>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {visit.resep.length > 0 && (
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider text-ink-soft mb-2">Resep Obat</p>
                <div className="space-y-2">
                  {visit.resep.map((r, i) => (
                    <div key={i} className="flex items-center justify-between rounded-xl bg-accent-soft/60 px-3.5 py-2.5">
                      <div>
                        <p className="text-xs font-bold text-primary-dark">{r.nama}</p>
                        <p className="text-[11px] text-ink-soft">{r.dosis}</p>
                      </div>
                      <span className="rounded-lg bg-white border border-line px-2 py-1 text-[11px] font-bold text-ink-soft">{r.jumlah}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {visit.resep.length === 0 && (
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider text-ink-soft mb-1.5">Resep Obat</p>
                <p className="text-xs italic text-ink-soft">Tidak ada resep untuk kunjungan ini.</p>
              </div>
            )}
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-ink-soft mb-1.5">Catatan Dokter</p>
              <p className="text-xs leading-relaxed text-ink bg-paper rounded-xl px-3.5 py-3 border border-line">{visit.catatanDokter}</p>
            </div>
            <div className="flex items-center justify-between rounded-xl bg-primary-soft px-4 py-3 border border-primary/10">
              <span className="text-xs font-semibold text-ink-soft">Total Biaya</span>
              <span className="text-sm font-black text-primary-dark">{formatCurrency(visit.biaya)}</span>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => alert(`Mengunduh resume kunjungan ${visit.id}...`)}
...
export default function RiwayatKunjunganPage() {
  ...
  const [selectedType, setSelectedType] = useState<VisitType | "Semua">("Semua");
    ...
    async function loadData() {
      try {
        ...
        if (patientData?.pasien) {
          ...
          const mappedVisits: VisitRecord[] = (patientData.pasien.history || []).map((k: {
            id?: string;
            tanggal?: string;
            facility?: string;
            faskesType?: string;
            ...
            if (actions.includes("lab") || actions.includes("darah") || actions.includes("urin") || actions.includes("pemeriksaan fisik")) {
              tipeKunjungan = "Pemeriksaan Lab";
            } else if (actions.includes("rontgen") || actions.includes("x-ray") || actions.includes("radiologi") || actions.includes("scan")) {
              tipeKunjungan = "Radiologi";
            } else if (actions.includes("rawat") || actions.includes("inap") || actions.includes("opname")) {
              tipeKunjungan = "Rawat Inap";
            } else if (k.resepObat && k.resepObat.length > 0 && !k.keluhan && !k.diagnosis) {
            ...
            if (Array.isArray(k.resepObat)) {
              k.resepObat.forEach((rStr: string) => {
                ...
                const cleanNameAndDosis = trimmed.replace(/\([^)]+\)/, "").trim();
                
                const dosageMatch = cleanNameAndDosis.match(/(\d+\s*(?:mg|mcg|ml|g|tab|cap|tablet))/i);
                const dosisVal = dosageMatch ? dosageMatch[1] : "Sesuai petunjuk";
                const namaVal = dosageMatch ? cleanNameAndDosis.replace(dosageMatch[1], "").trim() : cleanNameAndDosis;
            ...
            return {
              ...
              lokasiKota: k.faskesType === "RUMAH_SAKIT" ? "Rumah Sakit" : "Klinik / Puskesmas",
              dokter: k.doctorName || "Petugas Medis",
              spesialisasi: k.licenseNo ? `STR: ${k.licenseNo}` : "Petugas Medis",
              tipeKunjungan,
              ...
              resep,
              catatanDokter: k.tindakan ? `Tindakan: ${k.tindakan}.` : "Patuhi dosis resep obat.",
              biaya: 0 // BPJS / Free
            };
          });
  ...
  const years = useMemo(() => {
    const allYears = [...new Set(visits.map((v) => v.tanggal.slice(0, 4)))].sort((a, b) => Number(b) - Number(a));
  ...
  const filtered = useMemo(() => {
    return visits.filter((v) => {
      ...
      const matchesType = selectedType === "Semua" || v.tipeKunjungan === selectedType;
      const matchesYear = selectedYear === "Semua" || v.tanggal.startsWith(selectedYear);
      ...
      <div className="flex min-h-screen flex-col bg-paper">
        ...
        <main className="mx-auto w-full max-w-[1280px] flex-1 px-4 py-8 md:px-10">
          ...
          <div className="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="h-20 rounded-2xl border border-line bg-white p-4 shadow-sm animate-pulse flex items-center gap-3">
      ...
      <div className="flex min-h-screen flex-col bg-paper">
        ...
        <header className="sticky top-0 z-50 w-full border-b border-line bg-paper/90 backdrop-blur-sm">
          <div className="mx-auto flex max-w-[1280px] items-center justify-between px-4 py-3.5 md:px-10">
            ...
            <div className="flex items-center gap-3">
              <Link href="/login" className="rounded-xl bg-primary px-4 py-2 text-xs font-semibold text-white hover:bg-primary-dark">Masuk</Link>
            </div>
          </div>
        </header>
        ...
        <main className="mx-auto w-full max-w-[1280px] flex-1 px-4 py-16 md:px-10 flex flex-col items-center justify-center text-center">
          ...
          <h3 className="font-display text-lg font-bold text-primary-dark">Gagal Memuat Halaman</h3>
          <p className="mt-1.5 max-w-[400px] text-sm text-ink-soft leading-relaxed">{error}</p>
  ...
  const visitsThisYear = visits.filter(v => v.tanggal.startsWith(currentYearStr)).length.toString();
  const totalPrescriptions = visits.reduce((s, v) => s + v.resep.length, 0).toString();
  ...
  return (
    <div className="flex min-h-screen flex-col bg-paper">
        ...
        <div className="mx-auto flex max-w-[1280px] items-center justify-between px-4 py-3.5 md:px-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative h-9 w-9 overflow-hidden rounded-full border border-line shadow-sm">
              <Image src="/sarah-avatar.png" alt={patientName} fill className="object-cover" />
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto w-full max-w-[1280px] flex-1 px-4 py-8 md:px-10">
        ...
        {/* Page heading */}
        <div className="mb-8">
          <p className="font-mono text-xs font-semibold uppercase tracking-widest text-primary">Pasien · {patientName}</p>
          <h1 className="mt-1 font-display text-2xl font-bold tracking-tight text-primary-dark">Riwayat Kunjungan</h1>
          <p className="mt-1 text-sm text-ink-soft">Seluruh rekam medis perjalanan kesehatan kamu tersimpan di sini.</p>
        </div>

        {/* ── STAT CARDS ── */}
        <div className="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { label: "Total Kunjungan", value: totalVisitsCount, icon: <CalendarIcon className="h-4 w-4" />, color: "bg-[#0b3c5d] text-white" },
            { label: `Tahun Ini (${currentYearStr})`, value: visitsThisYear, icon: <ActivityIcon className="h-4 w-4" />, color: "bg-primary-soft text-primary" },
            { label: "Resep Diterima", value: totalPrescriptions, icon: <PillIcon className="h-4 w-4" />, color: "bg-accent-soft text-accent" },
            { label: "Total Pengeluaran", value: "BPJS / Gratis", icon: <FileTextIcon className="h-4 w-4" />, color: "bg-[#f0e6ff] text-[#7c3aed]" },
          ].map((s) => (
            <div key={s.label} className="flex items-center gap-3 rounded-2xl border border-line bg-white p-4 shadow-sm">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm font-bold text-primary-dark truncate">{s.value}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mb-4 flex items-center justify-between">
          <p className="text-xs text-ink-soft">
            Menampilkan <span className="font-bold text-ink">{filtered.length}</span> dari <span className="font-bold text-ink">{visits.length}</span> kunjungan
            {filtered.length > 0 && <span> · Total Pengeluaran: <span className="font-bold text-primary-dark">BPJS / Gratis</span></span>}
          </p>
          {(searchQuery || selectedType !== "Semua" || selectedYear !== "Semua") && (
            <button
              onClick={() => { setSearchQuery(""); setSelectedType("Semua"); setSelectedYear("Semua"); }}
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h3 className="font-display text-base font-bold text-primary-dark">Tidak Ada Kunjungan</h3>
            <p className="mt-1 text-sm text-ink-soft">Belum ada catatan riwayat kunjungan medis untuk akun ini.</p>
          </div>
        ) : (
              ...
              <div key={year}>
                ...
                <div className="mb-4 flex items-center gap-3">
                  <div className="flex items-center gap-2 rounded-full bg-primary px-4 py-1.5 shadow-sm">
                    <span className="text-xs font-black text-white tracking-widest">{year}</span>
                  </div>
                  <div className="flex-1 h-px bg-line" />
                  <span className="text-xs font-semibold text-ink-soft">{yearVisits.length} kunjungan</span>
                </div>
        ...
        <div className="mt-10 flex items-center justify-center gap-2.5 rounded-2xl border border-line bg-white py-4 px-6 shadow-sm">
          ...
          <p className="text-xs text-ink-soft">
            Seluruh rekam medis disimpan dengan enkripsi <strong className="text-ink">AES-256-GCM</strong> dan hanya dapat diakses oleh faskes yang kamu izinkan.
          </p>
        </div>
      </main>
