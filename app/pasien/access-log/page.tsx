...
const baseMockLogs = [
  {
    ...
    isActive: true,
    accessedDetails: ["Melihat Hasil Lab Darah (Kolesterol)", "Melihat Riwayat Obat Rutin"],
  },
  {
    date: "Oct 15, 2023",
    time: "23:12 PM",
    ...
    isActive: false,
    accessedDetails: ["Melihat Riwayat Alergi (Penisilin)", "Melihat Kondisi Kronis"],
  },
  {
    date: "Sep 02, 2023",
    time: "14:20 PM",
    ...
    isActive: false,
    accessedDetails: ["Menambahkan Catatan Kunjungan Gigi", "Memperbarui Resep Obat (Ibuprofen)"],
  },
  {
    date: "Aug 18, 2023",
    time: "10:00 AM",
...
export default function AccessLogPage() {
  ...
  const handleRevoke = (id: number) => {
    if (confirm("Are you sure you want to revoke this session immediately?")) {
      setLogs((prevLogs) =>
        prevLogs.map((log) =>
          log.id === id
      ...
      <header className="sticky top-0 z-50 w-full border-b border-line bg-paper/90 backdrop-blur-sm">
        <div className="mx-auto flex max-w-[1280px] items-center justify-between px-4 py-3.5 md:px-10">
          ...
          <nav className="hidden items-center gap-8 md:flex h-full">
            ...
            <Link href="/petugas/workers" className="py-2 text-sm font-medium text-ink-soft hover:text-primary transition-colors">
              Healthcare Workers
            </Link>
            <Link href="/pasien/riwayat-kunjungan" className="py-2 text-sm font-medium text-ink-soft hover:text-primary transition-colors">
              Medical History
            </Link>
          </nav>
        ...
        <div className="rounded-2xl border border-line bg-white shadow-sm overflow-hidden">
                ...
                {paginatedLogs.map((log) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      <div className="font-bold text-primary-dark mb-1">{log.date}</div>
                      <div className="text-xs">{log.time}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="font-bold text-primary-dark mb-1">{log.facility}</div>
                      <div className="text-xs">{log.location}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="font-bold text-primary-dark mb-1">{log.staff}</div>
                      <div className="text-xs">{log.role}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold ${log.accessColor} mb-1.5`}>
                      ...
                      <div className={`text-[10px] font-semibold ${log.isActive ? "text-primary" : "text-ink-soft/70"}`}>
                        {log.duration}
                      </div>
                    </td>
                    <td className="px-6 py-4 text-right">
                      {log.isActive ? (
                        <button
                          onClick={(e) => { e.stopPropagation(); handleRevoke(log.id); }}
                  ...
                  {expandedRowId === log.id && (
                    <tr className="bg-[#f4f8fa]/50">
                      <td colSpan={6} className="px-6 py-4 border-l-2 border-primary">
                        <div className="pl-10">
                          <p className="text-xs font-bold text-primary-dark mb-2">Detail Data Medis yang Diakses:</p>
                          <ul className="space-y-1.5">
                            {log.accessedDetails?.map((detail, idx) => (
                              <li key={idx} className="text-xs text-ink-soft flex items-center gap-2">
                                <svg className="h-3 w-3 text-primary shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="flex items-center justify-between border-t border-line bg-white px-6 py-4">
            <p className="text-xs text-ink-soft font-medium">
              Showing <span className="font-bold text-primary-dark">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="font-bold text-primary-dark">{Math.min(currentPage * itemsPerPage, filteredLogs.length)}</span> of <span className="font-bold text-primary-dark">{filteredLogs.length}</span> entries
            </p>
        ...
        <div className="mx-auto grid max-w-[1280px] grid-cols-1 gap-10 px-4 py-12 md:grid-cols-12 md:px-10">
          <div className="col-span-12 md:col-span-5">
            ...
            <p className="mt-3 max-w-[360px] text-xs leading-relaxed text-ink-soft">
              Secure, portable, and patient-controlled medical records for a healthier future.
            </p>
