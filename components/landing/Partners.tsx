...
export function Partners() {
  return (
    <section id="mitra" className="mx-auto max-w-[1280px] px-4 py-8 md:px-10 animate-fade-in-up">
      <div className="rounded-3xl border border-line/70 bg-white/50 p-8 md:p-10 backdrop-blur-xs shadow-[0_8px_30px_rgba(26,58,110,0.02)]">
        <p className="mb-8 text-center font-mono text-[10px] font-bold uppercase tracking-[0.15em] text-ink-soft/70">
          Disiapkan untuk Integrasi Lintas Fasilitas Kesehatan
        </p>
        <div className="flex flex-wrap items-center justify-center gap-x-12 gap-y-6">
          {partners.map(({ icon: Icon, name, color, bg }) => (
            <div
              key={name}
              className={`flex items-center gap-3 rounded-2xl border border-line/30 bg-white/80 px-5 py-3 text-ink-soft shadow-xs transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/20 hover:shadow-[0_12px_24px_rgba(26,58,110,0.04)] ${color} group cursor-pointer`}
            >
